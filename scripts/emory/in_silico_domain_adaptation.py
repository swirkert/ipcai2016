...
class AdaptedPhysiologicalParameterPlot(luigi.Task):
...
def evaluate_data(df_train, w_train, df_validate, df_test, w_test,
    ...
    for one_w_train, one_w_test in zip(w_train, w_test):
        ...
        X_validate, y_validate = preprocessing(df_validate, snr=one_w_test)
        X_train, y_train = preprocessing(df_train)
        # now get adapted dataset
        X_sampled, y_sampled = sample(X_train, y_train, X_validate,
                                      step_size=1, window_size=(1, 1))
        # now add snr to unspoiled samples
        X_sampled = add_snr(X_sampled, one_w_test)
...
def main(args):
    ...
    w.add(AdaptedPhysiologicalParameterPlot(which_train=train, which_validate=validate,
                                            which_test=test, train_snr=10.,
